...
struct plotEntryType
{
  plotEntryType(const TString& name)
    : name_(name),
      ...
      graphUnbinned_(0)
  {
    float pTbin_[] = {0., 20., 40., 60., 80., 100., 120., 140., 160., 200., 250., 350., 600., 1000.};
    ...
    numeratorTauPtL_             = bookHistogram("numeratorTauPtL",             name, 13, pTbin_);
    ...
    denominatorTauPtL_           = bookHistogram("denominatorTauPtL",           name, 13, pTbin_);
                ...
                const std::string& outputFileName)
{
  ...
  if ( graph6 ) legend->AddEntry(graph6, legendEntry5.data(), "pl");
  legend->Draw();
  ...
  canvas->Update();
  std::string outputFileName_plot = "plots_Sep2016_genJet_v3/";
...
std::map<std::string, std::map<std::string, plotEntryType*> > makePlots(const TString& inputFileName, const std::string& name, int mode)
{
  ...
  Float_t genJetEn, genJetPx, genJetPy, genJetPz;
  Int_t   genJetMatch;
  ...
  tree->SetBranchAddress("genJetEn", &genJetEn);
  ...
  tree->SetBranchAddress("genJetDeltaR", &genJetDeltaR);
  ...
  */
  ...
  plotEntryMap["Jet"]["byVLooseIsolationMVArun2v1DBoldDMwLT"]          = plotsJet_byVLooseIsolationMVArun2v1DBoldDMwLT;
  ...
  plotEntryMap["Jet"]["byVVTightIsolationMVArun2v1DBoldDMwLT"]         = plotsJet_byVVTightIsolationMVArun2v1DBoldDMwLT;
  ...
  for ( int iEntry = 0; iEntry < numEntries && (iEntry < maxEvents || maxEvents == -1); ++iEntry ) {
    ...
    TLorentzVector genJetP4(genJetPx, genJetPy, genJetPz, genJetEn);
    Float_t genJetPt  = genJetP4.Pt();
    Float_t genJetEta = ( genJetPt > 1.e-3 ) ? genJetP4.Eta() : 9.9;
    Float_t genJetPhi = genJetP4.Phi();
...
//    }
    ...
    if ( genJetPt > 20. && TMath::Abs(genJetEta) < 2.3){ // && genJetMatch > 0.5 && genJetDeltaR < 0.3 && recTauPt > 18.) { 
 
      bool genJetPreselection_passed = 
	(recTauPt > 20. && TMath::Abs(recTauEta) < 2.3 && genJetMatch > 0.5 && genJetDeltaR < 0.3);
      bool genJetOldDecayMode_passed = decayModeFindingOldDMs > 0.5 &&
	(recTauDecayMode == -1 || recTauDecayMode == 0 || recTauDecayMode == 1 || recTauDecayMode == 2 || recTauDecayMode == 10);
      bool genJetNewDecayMode_passed = decayModeFindingNewDMs > 0.5 &&
        (recTauDecayMode == -1 || recTauDecayMode == 0 || recTauDecayMode == 1 || recTauDecayMode == 2 || recTauDecayMode == 5 || recTauDecayMode == 6 || recTauDecayMode == 10);
      //bool genJetNewDecayMode_passed = genJetOldDecayMode_passed;

      double genJetEtaImpactECAL = genJetEta;
     
      plotsJet_byDecayModeFindingNewDMs->fillHistograms(
	genJetPreselection_passed && decayModeFindingNewDMs > 0.5, true,  
	genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byDecayModeFindingOldDMs->fillHistograms(
	genJetPreselection_passed && decayModeFindingOldDMs > 0.5, true,  
	genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);

      plotsJet_byCombinedIsolation3HitsLoose->fillHistograms(
	genJetPreselection_passed && genJetOldDecayMode_passed && byLooseCombinedIsolationDeltaBetaCorr3Hits > 0.5, true,  
	genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byCombinedIsolation3HitsMedium->fillHistograms(
	genJetPreselection_passed && genJetOldDecayMode_passed && byMediumCombinedIsolationDeltaBetaCorr3Hits > 0.5, true,  
	genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byCombinedIsolation3HitsTight->fillHistograms(
	genJetPreselection_passed && genJetOldDecayMode_passed && byTightCombinedIsolationDeltaBetaCorr3Hits > 0.5, true,  
        genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);

      plotsJet_byCombinedIsolationNewDM3HitsLoose->fillHistograms(
	genJetPreselection_passed && genJetNewDecayMode_passed && byLooseCombinedIsolationDeltaBetaCorr3Hits > 0.5, true,
	genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byCombinedIsolationNewDM3HitsMedium->fillHistograms(
	genJetPreselection_passed && genJetNewDecayMode_passed && byMediumCombinedIsolationDeltaBetaCorr3Hits > 0.5, true,
	genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byCombinedIsolationNewDM3HitsTight->fillHistograms(
	genJetPreselection_passed && genJetNewDecayMode_passed && byTightCombinedIsolationDeltaBetaCorr3Hits > 0.5, true,
	genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);

      plotsJet_byCombinedIsolation3HitsdR03Loose->fillHistograms(
        genJetPreselection_passed && genJetOldDecayMode_passed && byLooseCombinedIsolationDeltaBetaCorr3HitsdR03 > 0.5, true,
	genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byCombinedIsolation3HitsdR03Medium->fillHistograms(
	genJetPreselection_passed && genJetOldDecayMode_passed && byMediumCombinedIsolationDeltaBetaCorr3HitsdR03 > 0.5, true,
	genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byCombinedIsolation3HitsdR03Tight->fillHistograms(
	genJetPreselection_passed && genJetOldDecayMode_passed && byTightCombinedIsolationDeltaBetaCorr3HitsdR03 > 0.5, true,
	genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      
      plotsJet_byPileupWeightedIsolation3HitsLoose->fillHistograms(
	genJetPreselection_passed && genJetOldDecayMode_passed && byLoosePileupWeightedIsolation3Hits > 0.5, true,
	genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byPileupWeightedIsolation3HitsMedium->fillHistograms(
	genJetPreselection_passed && genJetOldDecayMode_passed && byMediumPileupWeightedIsolation3Hits > 0.5, true,
	genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byPileupWeightedIsolation3HitsTight->fillHistograms(
	genJetPreselection_passed && genJetOldDecayMode_passed && byTightPileupWeightedIsolation3Hits > 0.5, true,
	genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);

      plotsJet_byPileupWeightedIsolationNewDM3HitsLoose->fillHistograms(
	genJetPreselection_passed && genJetNewDecayMode_passed && byLoosePileupWeightedIsolation3Hits > 0.5, true,
	genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byPileupWeightedIsolationNewDM3HitsMedium->fillHistograms(
	genJetPreselection_passed && genJetNewDecayMode_passed && byMediumPileupWeightedIsolation3Hits > 0.5, true,
	genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byPileupWeightedIsolationNewDM3HitsTight->fillHistograms(
	genJetPreselection_passed && genJetNewDecayMode_passed && byTightPileupWeightedIsolation3Hits > 0.5, true,
	genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);

      plotsJet_byChargedIsolationLoose->fillHistograms(
	genJetPreselection_passed && genJetOldDecayMode_passed && chargedIsoPtSum < 2.0, true,  
	genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byChargedIsolationMedium->fillHistograms(
	genJetPreselection_passed && genJetOldDecayMode_passed && chargedIsoPtSum < 1.0, true,  
	genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byChargedIsolationTight->fillHistograms(
	genJetPreselection_passed && genJetOldDecayMode_passed && chargedIsoPtSum < 0.8, true,  
	genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);

      plotsJet_byVLooseIsolationMVArun2v1DBoldDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetOldDecayMode_passed && byVLooseIsolationMVArun2v1DBoldDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byLooseIsolationMVArun2v1DBoldDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetOldDecayMode_passed && byLooseIsolationMVArun2v1DBoldDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byMediumIsolationMVArun2v1DBoldDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetOldDecayMode_passed && byMediumIsolationMVArun2v1DBoldDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byTightIsolationMVArun2v1DBoldDMwLT->fillHistograms(
         genJetPreselection_passed && genJetOldDecayMode_passed && byTightIsolationMVArun2v1DBoldDMwLT, true,
         genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byVTightIsolationMVArun2v1DBoldDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetOldDecayMode_passed && byVTightIsolationMVArun2v1DBoldDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byVVTightIsolationMVArun2v1DBoldDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetOldDecayMode_passed && byVVTightIsolationMVArun2v1DBoldDMwLT, true,
         genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);

      plotsJet_byVLooseIsolationMVArun2v1DBnewDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetNewDecayMode_passed && byVLooseIsolationMVArun2v1DBnewDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byLooseIsolationMVArun2v1DBnewDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetNewDecayMode_passed && byLooseIsolationMVArun2v1DBnewDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byMediumIsolationMVArun2v1DBnewDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetNewDecayMode_passed && byMediumIsolationMVArun2v1DBnewDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byTightIsolationMVArun2v1DBnewDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetNewDecayMode_passed && byTightIsolationMVArun2v1DBnewDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byVTightIsolationMVArun2v1DBnewDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetNewDecayMode_passed && byVTightIsolationMVArun2v1DBnewDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
   //plotsJet_byVVTightIsolationMVArun2v1DBnewDMwLT->fillHistograms(
   //	 genJetPreselection_passed && genJetNewDecayMode_passed && byVVTightIsolationMVArun2v1DBnewDMwLT, true,
   //	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);

      plotsJet_byVLooseIsolationMVArun2v1PWoldDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetOldDecayMode_passed && byVLooseIsolationMVArun2v1PWoldDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byLooseIsolationMVArun2v1PWoldDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetOldDecayMode_passed && byLooseIsolationMVArun2v1PWoldDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byMediumIsolationMVArun2v1PWoldDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetOldDecayMode_passed && byMediumIsolationMVArun2v1PWoldDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byTightIsolationMVArun2v1PWoldDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetOldDecayMode_passed && byTightIsolationMVArun2v1PWoldDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byVTightIsolationMVArun2v1PWoldDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetOldDecayMode_passed && byVTightIsolationMVArun2v1PWoldDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
//plotsJet_byVVTightIsolationMVArun2v1PWoldDMwLT->fillHistograms(
//	 genJetPreselection_passed && genJetOldDecayMode_passed && byVVTightIsolationMVArun2v1PWoldDMwLT, true,
//	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);

      plotsJet_byVLooseIsolationMVArun2v1PWnewDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetNewDecayMode_passed && byVLooseIsolationMVArun2v1PWnewDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byLooseIsolationMVArun2v1PWnewDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetNewDecayMode_passed && byLooseIsolationMVArun2v1PWnewDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byMediumIsolationMVArun2v1PWnewDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetNewDecayMode_passed && byMediumIsolationMVArun2v1PWnewDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byTightIsolationMVArun2v1PWnewDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetNewDecayMode_passed && byTightIsolationMVArun2v1PWnewDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byVTightIsolationMVArun2v1PWnewDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetNewDecayMode_passed && byVTightIsolationMVArun2v1PWnewDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
//plotsJet_byVVTightIsolationMVArun2v1PWnewDMwLT->fillHistograms(
//	 genJetPreselection_passed && genJetNewDecayMode_passed && byVVTightIsolationMVArun2v1PWnewDMwLT, true,
//	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);

      plotsJet_byVLooseIsolationMVArun2v1DBdR03oldDMwLT->fillHistograms(
         genJetPreselection_passed && genJetOldDecayMode_passed && byVLooseIsolationMVArun2v1DBdR03oldDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byLooseIsolationMVArun2v1DBdR03oldDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetOldDecayMode_passed && byLooseIsolationMVArun2v1DBdR03oldDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byMediumIsolationMVArun2v1DBdR03oldDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetOldDecayMode_passed && byMediumIsolationMVArun2v1DBdR03oldDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byTightIsolationMVArun2v1DBdR03oldDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetOldDecayMode_passed && byTightIsolationMVArun2v1DBdR03oldDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byVTightIsolationMVArun2v1DBdR03oldDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetOldDecayMode_passed && byVTightIsolationMVArun2v1DBdR03oldDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
//      plotsJet_byVVTightIsolationMVArun2v1DBdR03oldDMwLT->fillHistograms(
//	 genJetPreselection_passed && genJetOldDecayMode_passed && byVVTightIsolationMVArun2v1DBdR03oldDMwLT, true,
//	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);

      plotsJet_byVLooseIsolationMVArun2v1PWdR03oldDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetOldDecayMode_passed && byVLooseIsolationMVArun2v1PWdR03oldDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byLooseIsolationMVArun2v1PWdR03oldDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetOldDecayMode_passed && byLooseIsolationMVArun2v1PWdR03oldDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byMediumIsolationMVArun2v1PWdR03oldDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetOldDecayMode_passed && byMediumIsolationMVArun2v1PWdR03oldDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byTightIsolationMVArun2v1PWdR03oldDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetOldDecayMode_passed && byTightIsolationMVArun2v1PWdR03oldDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
      plotsJet_byVTightIsolationMVArun2v1PWdR03oldDMwLT->fillHistograms(
	 genJetPreselection_passed && genJetOldDecayMode_passed && byVTightIsolationMVArun2v1PWdR03oldDMwLT, true,
	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
//      plotsJet_byVVTightIsolationMVArun2v1PWdR03oldDMwLT->fillHistograms(
//	 genJetPreselection_passed && genJetOldDecayMode_passed && byVVTightIsolationMVArun2v1PWdR03oldDMwLT, true,
//	 genJetPt, genJetEta, genJetEtaImpactECAL, genJetPhi, leadPFChargedHadrCandPt, leadPFChargedHadrCandEta, tauNeutralHadPt, recTauDecayMode, 0, numVertices, evtWeight_dummy);
...
void makeTauIdEffPlots2_newMVAId_Fake()
{
  gROOT->SetBatch(true);
  ...
  //inputFileName_newTauId.Append("QCD_Pt-15to7000_Flat_13TeV_Fall15_25ns_miniAODv2_v1.root");
  TString inputFileName_newTauId = "/nfs/dust/cms/user/anayak/CMS/OnSLC6/CMSSW_763_tauId/src/TauAnalysis/Test/test/";
  inputFileName_newTauId.Append("pfTauIdEffNtupleFromMiniAOD_Fall15_QCD_run2_v2.root");
...
//             "makeTauIdEffPlots_newMVAId_Taus_run2MVA1isoPWdR03oldDMwLT.pdf");
  ...
  */
  showGraphs(800, 600,
	     plots_newTauId["Jet"]["byVLooseIsolationMVArun2v1DBoldDMwLT"], legenEntry_vloose.data(),
	     ...
	     plots_newTauId["Jet"]["byVVTightIsolationMVArun2v1DBoldDMwLT"], legenEntry_vvtight.data(),
             0.045, 0.68, 0.60, 0.20, 0.25,
             "QCD", 0.055,
             0.74, 0.16, 0.15, 0.06,
             true, 5.e-5, 5.e-1, "Fake-rate", 1.2,
